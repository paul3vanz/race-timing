...
import { FacadeVFS } from '../node_modules/expo-sqlite/web/wa-sqlite/FacadeVFS.js';
import * as VFS from '../node_modules/expo-sqlite/web/wa-sqlite/VFS.js';
...
export class AccessHandlePoolVFS extends FacadeVFS {
  ...
  async #acquireAccessHandles() {
    ...
    // A file that throws NoModificationAllowedError has a handle held by a
    // still-running worker from a previous Expo Fast Refresh cycle. Skip it —
    // the pool will create fresh capacity below if needed, and the stale file
    // will become available again once that worker terminates.
    await Promise.all(
      files.map(async ([name, handle]) => {
        try {
          ...
          // Stale handle from a previous session — skip this pool file.
        }
      })
    );
  }
